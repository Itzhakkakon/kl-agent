...
class MacKeyLogger(IKeyLogger):
    ...
    def stop_async(self):
        """Stops the key logging process asynchronously."""
        if self.active:
            self.active = False
            Quartz.CFRunLoopStop(Quartz.CFRunLoopGetCurrent())

    def get_pressed_keys(self):
        """Returns a copy of logged keys."""
        return list(self.log)

    def callback(self, proxy, event_type, event, refcon):
        """Callback function to capture keystrokes."""
    ...
    def key_code_to_string(self, key_code):
        """Converts key codes to readable strings."""
        key_map = {
